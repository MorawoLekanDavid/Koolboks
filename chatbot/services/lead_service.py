...
async def save_lead(user_name: str, phone: str, history: list, session_id: str = None):
    ...
    try:
        prompt = (
            ...
            "- product_interest: ONLY the exact product the CUSTOMER explicitly chose or "
            "confirmed (e.g. they said 'I'll take that one', named a specific size/model, "
            "or clearly agreed to it). If the bot only recommended or listed products and "
            "the customer never actually picked one — even if they gave their phone number "
            "right after — leave this EMPTY. Do not guess based on whatever the bot mentioned "
            "last.\n"
            "- amount: price of that chosen product as mentioned in the chat, or empty if "
            "product_interest is empty\n"
